"""Append-only Parquet storage — history is never deleted."""
...
class ParquetStore:
    """Partitioned append-only store: data/{kind}/{symbol}/{date}.parquet"""
    ...
    def buffer(self, kind: str, row: dict[str, Any]) -> None:
        key = f"{kind}:{row.get('inst_id', row.get('symbol', 'unknown'))}"
    ...
    def _append_rows(self, kind: str, symbol: str, rows: list[dict[str, Any]]) -> int:
        ...
        if ts_col not in df_new.columns:
            log.warning("No timestamp column in %s rows", kind)
            return 0
    ...
    def _append_rows_for_day(
        ...
        if path.exists():
            try:
                ...
                dedup_cols = [c for c in ("ts", "seq_id", "trade_id") if c in df.columns]
                if dedup_cols:
                    df = df.drop_duplicates(subset=dedup_cols, keep="last")
                else:
                    df = df.drop_duplicates(subset=[ts_col], keep="last")
            except Exception as e:
                log.warning("Read parquet failed %s: %s — writing new only", path, e)
                df = df_new
        else:
            df = df_new
        ...
        log.info("Appended %d rows -> %s (total %d)", len(df_new), path, len(df))
        return len(df_new)
